                ...
                FROM (
                    ...
                    INNER JOIN (
                        ...
                        LEFT JOIN (
                            SELECT
                                author_id,
                                count(author_id) as borehole_author
                            FROM
                                bdms.borehole
                            GROUP BY
                                author_id
                        ) as bha
                        ON id_usr = bha.author_id

                        LEFT JOIN (
                            SELECT
                                updater_bho,
                                count(updater_bho) as borehole_updater
                            FROM
                                bdms.borehole
                            GROUP BY
                                updater_bho
                        ) as bhu
                        ON id_usr = bhu.updater_bho

                        LEFT JOIN (
                            SELECT
                                locked_by,
                                count(locked_by) as borehole_locker
                            FROM
                                bdms.borehole
                            GROUP BY
                                locked_by
                        ) as bhl
                        ON id_usr = bhl.locked_by
